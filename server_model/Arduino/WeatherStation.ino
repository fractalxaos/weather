...
#define HTTP_NO_REQUEST_TIMEOUT 120 // milliseconds
...
volatile long lastRainClickTime;
// wind
volatile byte irqWindClicks;
volatile long lastWindClickTime;

// Housekeeping
byte seconds;
byte minutes;
long lastLoopCycleTime;
long lastWindCheckTime;
byte ptr2mWind;
byte ptr1hRain;
byte ptr10mGust;
...
void WindSpeedIRQ() {
   ...
   * Returns nothing.
   */
  long currentTime; // system time (msec) when interrupt handler called

  currentTime = millis();
  if (abs(currentTime - lastWindClickTime) > WIND_SENSOR_DEBOUNCE_SETTLING_TIME) {
...
void RainFallIRQ() {
  ...
  currentTime = millis();
  if (abs(currentTime - lastRainClickTime) > RAIN_SENSOR_DEBOUNCE_SETTLING_TIME) {
...
void setup() {
  ...
  Serial.begin(9600);
  Serial.println(F("\nDIY weather v2.1"));
  Serial.println(F("(c) 2017 intravisions.com\n"));
  ...
  aPressureSensor.begin(); // Get sensor online
  aPressureSensor.setModeBarometer(); // Measure pressure in Pascals from 20 to 110 kPa
  aPressureSensor.setOversampleRate(7); // Set Oversample to the recommended 128
  aPressureSensor.enableEventFlags(); // Enable all three pressure and temp event flags 
...
void loop() {
   ...
   * Returns nothing.
   */
  long currentTime; // stores current system time (msec)
...
void update2minWindAverage() {
   ...
   * Returns nothing.
   */
  arrWindSpeed2mAv[ptr2mWind] = byte(windSpeed + 0.5);
...
void update1secWindGust() {
  /*
   * Called once every second, this function Updates the daily wind gust data and the 
   * ten minute wind gust data for the current minute.  Each second the instantanious 
   * wind speed and direction get measured by wind sensors.  If the instantainous wind 
   * speed is greater than the reading from the previous second, then the daily wind 
   * gust data gets updated with the new reading.  Similarily, the 10 minute wind gust 
   * data also gets updated as follows.  Each element of the 10 minuite wind gust array 
   * represents the maximum wind speed measured during a one minute interval.  Hence, taking 
   * the maximum of the array gives the maximum wind gust measured during the ten minute 
   * interval ending at the current time.  As with the daily wind gust, once every second 
   * the element representing the current minute gets updated if the instantanious wind speed 
   * is greater than the previous value stored in the array element.  Note that both the wind 
   * speed and wind direction arrays get updated at the same time.
   * 
   * Parameters: none
   * Returns nothing.
   */
  // update the current minute in the ten minute wind gust arrays
  if (windSpeed > float(arrWindGustSpeed10m[ptr10mGust])) {
    arrWindGustSpeed10m[ptr10mGust] = byte(windSpeed);
    arrWindGustDirection10m[ptr10mGust] = windDirection;
  }

  // update the daily wind gust data items
  if (windSpeed > float(windGustSpeed)) {
    windGustSpeed = byte(windSpeed);
...
void update1hourRainFall() {
  /* 
   * Updates the rain fall value for the current minute.  Sixty minutes of data
   * kept in order to calculate the total rain fall for the last hour. Note that
   * the rain fall sensor is calibrated to produce a switch closure (or tick) for 
   * every 0.011 inches of rain fall.  Hence rain fall per minute is given by
   * the number of ticks per minute times 0.011, and the rain fall per hour is
   * this number multiplied by sixty.
   * 
   * Parameters: none
   * Returns nothing.
   */
...
void calcWeather() {
  /*
   * Always called before reporting the weather to the destination server, this
   * routine calculates weather data items.  In summary, the following items
   * get calculated:
   *     - two minute average wind speed and direction
   *     - the highest wind gust in the last ten minutes
...
byte getWindSpeed() {
  ...
  clicksPerSecond = 1000.0 * float(irqWindClicks) / float(abs(currentTime - lastWindCheckTime));
...
byte getWindDirection() {
  /*
   * Calculates the wind direction from raw sensor data.  The wind direction sensor
   * measures to an accuracy of +/- 11.25 degrees, or of one compass point.  Hence,
   * the raw data from the sensor gets converted to one of 16 compass points expressed 
   * by an integer (0, 1, ... 15).
   *     0 for N
   *     1 for NNE
   *     ...
   *     15 for NNW
  ...
  adc = averageAnalogRead(WIND_DIRECTION); // get the current reading from the sensor

  // this helps to determine if the wind direction sensor is working properly
  #ifdef DEBUG
    Serial.print(F(" raw "));Serial.println(adc);
  #endif
  /*
   * The following are the decision points for each of 16 possible directions reported by the
   * wind diretion sensor.  Each decision point is half way between the ADC reading for the two
   * adjacent wind directions.  Note that the wind directions reported by the wind sensor are not
   * in the same order as the decision points.  See the wind sensor documentation for details.
  */
  ...
  return (16); // a disconnected wind sensor results in an max ADC reading of 1023
}

float getLightLevel() {
/* 
 *  Calculates the voltage of the light sensor based on 3.3 volts, thus allowing 
 *  for a VCC of either 3.3 or 5.0 volts.  (An Arduino plugged into a USB port 
 *  has a VCC of 4.5 to 5.2V).
 *  
 *  Parameters: none
 ...
 */
  ...
  referenceVoltage = 3.25 / referenceVoltage; //The reference voltage is 3.25V
...
float getBatteryLevel() {
  /*
   * Calculates the power supply voltage based on the 3.3 volts, thus allowing 
   * for a VCC of either 3.3 or 5.0 volts. The power supply voltage is applied
   * to a voltage divider consisting of two 5% resistors: 3.9K on the high side 
   * (R7), and 1K on the low side (R8).  The Arduino reads the voltage on the
   * low side via a gpio analog input (pin A2).
   * 
   * Parameters: none
  ...
  // normalize the reference voltage ADC reading to 3.3 volts
  referenceVoltage = 3.25 / referenceVoltage;
  // multiply battery voltage ADC reading by the normalized reference voltage
  batteryVoltage = batteryVoltage * referenceVoltage;
    
  // calculate the acutal battery voltage across the voltage divider
  // v = (R7 + R8) / R7 = (3.9K + 1.0K)/1.0K = 4.9 times normalized battery voltage
  // + 0.57 correction added to get true battery voltage
  batteryVoltage = batteryVoltage * 4.97 + 0.57;
  ...
  if (client) 
  {
    ...
    switch (i) {
      ...
      case 0:
        ...
        sprintf(sTmp, "wgs=%d,", windGustSpeed);
        strcat(sBuf, sTmp);
      
        sprintf(sTmp, "wgd=%d,", windGustDirection);
        strcat(sBuf, sTmp);
      
        sprintf(sTmp, "wgs10=%d,", windGustSpeedMax10m);
        strcat(sBuf, sTmp);
      
        sprintf(sTmp, "wgd10=%d,", windGustDirectionMax10m);
        strcat(sBuf, sTmp);
...
byte processCommand(char * sBuf) {
  ...
  switch(*(pStr + 0)) {
    ...
    case 'w': // change wifi wpa password
      pStr = strtok(NULL, "/");
      if(pStr == NULL)
        return 5;
      sReadEeprom(sTmp, STATION_PIN);
...
void initializeWifiAdapter()
{
  ...
  */
  ...
  /*
   * Note that esp8266.begin() returns a boolean false value if the 
   * wifi adapter is not available or operating correctly.
   */
  if (result == false) {
...
void connectWifiAccessPoint() {
  ...
  Serial.println(ssid);
  /* 
   * The esp8266.connect([ssid], [psk]) function returns 
   * a value > 0, if successful.  Otherwise, on fail:
   *     -1: TIMEOUT - the default timeout is 30 seconds
   *     -3: FAIL - cannot connect to wifi access point
   */
